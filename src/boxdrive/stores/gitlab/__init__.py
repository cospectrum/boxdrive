...
FilterV1 = Callable[[list[ObjectInfo]], ListObjectsInfo]
FilterV2 = Callable[[list[ObjectInfo]], ListObjectsV2Info]
...
class GitlabStore(ObjectStore):
    ...
    async def _list_object_keys(
        ...
        for page in range(1, MAX_PAGE):
            ...
            for item in items:
                if item.name == self.placeholder_name:
                    continue
    ...
    async def list_objects(
        ...
        *,
        ...
        filter_: FilterV1 = lambda objects: filter_objects(
            objects,
            prefix=prefix,
            delimiter=delimiter,
            max_keys=max_keys,
        ...
        def is_enough(keys: list[Key]) -> bool:
            objects = [default_object_info(key) for key in keys]
            return filter_(objects).is_truncated

        keys = await self._list_object_keys(bucket_name, max_keys, is_enough)
        objects = []
        for key in keys:
            obj = await self.head_object(bucket_name, key)
            objects.append(obj)
        return filter_(objects)
    ...
    async def list_objects_v2(
        ...
        *,
        ...
        """List objects in a bucket."""
        filter_: FilterV2 = lambda objects: filter_objects_v2(
            objects,
